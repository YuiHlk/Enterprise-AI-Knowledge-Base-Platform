...
import com.test.qa.dto.RunEvaluationResponse;
import com.test.qa.dto.TestSetRequest;
import com.test.qa.dto.TestSetResponse;
...
public class EvaluationController {
    ...
    public Result<List<TestSetResponse>> getQuestions(
            @Parameter(description = "评测集名称") @PathVariable String setName) {
        return Result.success(EvaluationConverter.toTestSetResponseList(
                testSetService.getQuestionsBySetName(setName)));
    }

    @PostMapping("/test-sets")
    @Operation(summary = "创建评测问题（支持批量）",
            description = "请求体为TestSetRequest数组，setName相同的为同一批次")
    public Result<List<TestSetResponse>> createQuestions(
            @Parameter(description = "评测问题列表") @RequestBody List<TestSetRequest> requests) {
        if (requests == null || requests.isEmpty()) {
            return Result.error(400, "问题列表不能为空");
        }
        return Result.success(EvaluationConverter.toTestSetResponseList(
                testSetService.batchCreate(EvaluationConverter.toTestSetEntityList(requests))));
    }
    ...
    public Result<RunEvaluationResponse> runEvaluation(
        ...
        return Result.success(new RunEvaluationResponse(taskId, "STARTED"));
    }

    @GetMapping("/tasks")
    @Operation(summary = "获取所有评测任务列表（按taskId分组）")
    public Result<List<EvaluationTaskItem>> listTasks() {
    ...
    public Result<EvaluationTaskDetailResponse> getTaskDetail(
            @Parameter(description = "任务ID") @PathVariable String taskId) {
        return Result.success(evaluationService.getTaskSummary(taskId));
    }

    @GetMapping("/records/{id}")
    @Operation(summary = "获取单条评测记录详情")
    public Result<EvaluationRecordResponse> getRecord(
